...
function repeatString(str, times) {
  if ((typeof str === 'string' || str instanceof String) && times > 0) {
    return str.length > 0 ? str.repeat(times) : str;
  }
  return '';
}
...
function reverseWords(str) {
  ...
  if (str) {
    ...
    for (let i = 0; i < strList.length; i += 1) {
      newStr.push(strList[i].split('').reverse().join(''));
    }
  }
  return newStr.join(' ');
}
...
function unbracketTag(str) {
  const tagName = str.split('<').at(-1).split('>')[0];
  return tagName;
}
